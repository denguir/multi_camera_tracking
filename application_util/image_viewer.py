...
class MultiImageViewer(object):
    """An multi-image viewer with drawing routines and video capture capabilities.
    ...
    * 'ESC' : quit
    ...
    def __init__(self, update_ms, window_shapes, caption="Figure", n_cams=1):
        self.n_cams = n_cams
        self.viewers = [ImageViewer(update_ms, window_shape, caption=f"{caption} #{i}")
                       for i, window_shape in enumerate(window_shapes)]
        self.win_pos = list(range(0, win_shape, win_shape))
        self.images = np.zeros(self._window_shape + (3, ), dtype=np.uint8)
    
    def display(self, win_name, frame, position):
        cv2.namedWindow(win_name)
        cv2.moveWindow(win_name, position[0],position[1])
        cv2.imshow(win_name, resized_window1)
